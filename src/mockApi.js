const STORAGE_KEYS = {
  ...
  workOrders: 'axios_demo_work_orders',
...
const generateShareToken = () => {
  if (globalThis.crypto?.randomUUID) return globalThis.crypto.randomUUID().replace(/-/g, '').slice(0, 20);
...
const getState = () => ({
  ...
  workOrders: readJson(STORAGE_KEYS.workOrders, [])
});
...
const saveState = (state) => {
  ...
  writeJson(STORAGE_KEYS.workOrders, state.workOrders);
};
...
const withWorkOrderTotals = (workOrder) => {
  ...
  const laborTotal = Number(
    items.filter((item) => item.type === 'labor').reduce((acc, item) => acc + item.lineTotal, 0).toFixed(2)
  );
  const partsTotal = Number(
    items.filter((item) => item.type === 'part').reduce((acc, item) => acc + item.lineTotal, 0).toFixed(2)
  );
  ...
  return {
    ...
    totals: {
      laborTotal,
      partsTotal,
      grandTotal: Number((laborTotal + partsTotal).toFixed(2))
    }
  };
};

const nextOrderNumber = (workOrders) => {
  const datePart = toDateKey().replaceAll('-', '');
...
const buildSeedData = () => {
  ...
  return { serviceCenters, windows, requests: [], workOrders: [] };
};
...
const createWorkOrderFromRequest = async (requestId) => {
  await delay();
  const state = getState();
  const request = state.requests.find((item) => item.id === requestId);
  if (!request) return null;

  const existing = state.workOrders.find((item) => item.requestId === requestId);
  if (existing) return withWorkOrderTotals(existing);
  ...
  const workOrder = {
    ...
    title: 'Заказ-наряд',
    orderNumber: nextOrderNumber(state.workOrders),
    createdAt: nowIso,
    updatedAt: nowIso,
    status: 'draft',
    sentAt: null,
  ...
  saveState(state);
  return withWorkOrderTotals(workOrder);
};


const getWorkOrderByRequestId = async (requestId) => {
  await delay();
  const state = getState();
  const found = state.workOrders.find((item) => item.requestId === requestId);
  return found ? withWorkOrderTotals(found) : null;
};

const getWorkOrderById = async (id) => {
  await delay();
  const state = getState();
  const found = state.workOrders.find((item) => item.id === id);
  return found ? withWorkOrderTotals(found) : null;
};

const getWorkOrderByToken = async (token) => {
  await delay();
  const state = getState();
  const found = state.workOrders.find((item) => item.shareToken === token);
  return found ? withWorkOrderTotals(found) : null;
};

const updateWorkOrder = async (id, patch = {}) => {
  await delay();
  const state = getState();
  ...
  allowed.forEach((key) => {
    if (patch[key] !== undefined) workOrder[key] = patch[key];
  });
...
const addWorkOrderItem = async (orderId, item) => {
  await delay();
  const state = getState();
  const order = state.workOrders.find((entry) => entry.id === orderId);
  ...
  saveState(state);
  return withWorkOrderTotals(order);
};

const updateWorkOrderItem = async (orderId, itemId, patch = {}) => {
  await delay();
  const state = getState();
  const order = state.workOrders.find((entry) => entry.id === orderId);
  if (!order) return null;
  ...
  if (patch.type !== undefined) item.type = patch.type === 'part' ? 'part' : 'labor';
  if (patch.name !== undefined) item.name = String(patch.name).trim() || item.name;
  if (patch.qty !== undefined) item.qty = Number(patch.qty) > 0 ? Number(patch.qty) : item.qty;
  if (patch.unitPrice !== undefined) item.unitPrice = asMoney(patch.unitPrice);
...
const deleteWorkOrderItem = async (orderId, itemId) => {
  await delay();
  const state = getState();
  const order = state.workOrders.find((entry) => entry.id === orderId);
  if (!order) return null;
  ...
  if (before === order.items.length) return null;
...
const listWorkOrdersByServiceCenter = async (serviceCenterId) => {
  await delay();
  const state = getState();
  return state.workOrders
    .filter((item) => item.serviceCenterId === serviceCenterId)
    .map((item) => withWorkOrderTotals(item));
};

const markWorkOrderSent = async (orderId) => {
  await delay();
  const state = getState();
  const order = state.workOrders.find((entry) => entry.id === orderId);
  if (!order) return null;
...
export const mockApi = {
  ...
  getWorkOrderByRequestId,
  getWorkOrderById,
  getWorkOrderByToken,
  listWorkOrdersByServiceCenter,
  updateWorkOrder,
  addWorkOrderItem,
  updateWorkOrderItem,
  deleteWorkOrderItem,
