...
const withMetrics = (serviceCenter, requests) => {
  ...
  const confirmed = responded.filter((item) => item.status === 'confirmed');
  ...
  const confirmRate = responded.length ? Math.round((confirmed.length / responded.length) * 100) : null;
...
const updateRequestStatus = async (requestId, patch = {}) => {
  ...
  request.status = patch.status ?? request.status;
